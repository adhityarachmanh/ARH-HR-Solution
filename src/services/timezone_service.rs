...
pub async fn count_timezones(pool: &PgPool) -> Result<i64, AppError> {
    let count = sqlx::query_scalar!(r#"SELECT COUNT("TimeZoneId") FROM "TimeZones""#)
        .fetch_one(pool)
        .await
...
pub async fn get_all_timezones_paginated(
    pool: &PgPool,
    limit: i64,
    offset: i64,
) -> Result<Vec<TimeZone>, AppError> {
    let sql_query = format!(
        "SELECT {} FROM \"TimeZones\" ORDER BY \"TimeZoneName\" ASC LIMIT $1 OFFSET $2",
        TIMEZONE_FIELDS_SQL
    );

    let timezones = sqlx::query_as::<_, TimeZone>(&sql_query)
        .bind(limit)
        .bind(offset)
        .fetch_all(pool)
        .await
...
pub async fn create_timezone(pool: &PgPool, name: &str) -> Result<TimeZone, AppError> {
    let exists = sqlx::query_scalar!(
        "SELECT EXISTS(SELECT 1 FROM \"TimeZones\" WHERE LOWER(\"TimeZoneName\") = LOWER($1))",
        name
    )
    .fetch_one(pool)
    .await
    ...
    if exists.unwrap_or(false) {
        return Err(AppError::InternalError(
            "TimeZone already exists".to_string(),
        ));
    }
    ...
    info!(
        "TimeZone '{}' created with ID: {}",
        new_timezone.time_zone_name, new_timezone.time_zone_id
    );
    Ok(new_timezone)
}
...
pub async fn update_timezone(
    ...
    if exists.unwrap_or(false) {
        return Err(AppError::InternalError(
            "TimeZone name already exists".to_string(),
        ));
    }
    ...
    info!(
        "TimeZone ID {} updated to '{}'",
        updated_timezone.time_zone_id, updated_timezone.time_zone_name
    );
    Ok(updated_timezone)
}
...
pub async fn delete_timezone(pool: &PgPool, timezone_id: i32) -> Result<(), AppError> {
    let result = sqlx::query!(
        "DELETE FROM \"TimeZones\" WHERE \"TimeZoneId\" = $1",
        timezone_id
    )
    .execute(pool)
    .await
    ...
    if result.rows_affected() == 0 {
        return Err(AppError::InternalError(
            "TimeZone not found for deletion".to_string(),
        ));
    }
