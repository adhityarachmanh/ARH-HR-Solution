...
pub async fn list_master_permissions(
    ...
    let all_permissions: Vec<HardcodedPermission> =
        permission_service::get_all_permissions(pool.get_ref()).await?;
    ...
    let rendered = tera
        .render("permissions/list.html", &context)
...
pub async fn manage_permissions_form(
    ...
    let all_permissions: Vec<HardcodedPermission> =
        permission_service::get_all_permissions(pool.get_ref()).await?;
    let owned_permission_ids =
        permission_service::get_permissions_for_role(pool.get_ref(), role_id).await?;
    let current_username = get_username(pool.get_ref(), &_session).await;
    ...
    let rendered = tera
        .render("roles/permissions.html", &context)
        .map_err(AppError::TeraError)?;
    Ok(HttpResponse::Ok().body(rendered))
}
