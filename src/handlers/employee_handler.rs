...
use crate::models::{EmployeeFormData, PaginationParams};
use crate::services::{
    attendance_location_service, branch_service, class_service, employee_service,
    employment_status_service, grade_service, job_level_service, job_position_service,
...
async fn get_employee_id_from_path(path: web::Path<i32>) -> Result<i32, AppError> {
    ...
    if id <= 0 {
        return Err(AppError::BadRequest(
            "Employee ID must be greater than 0.".into(),
        ));
    }
    Ok(id)
}
...
async fn fetch_master_data(pool: &PgPool) -> Result<tera::Context, AppError> {
    ...
    context.insert(
        "organizations",
        &organization_service::get_all_organizations(pool).await?,
    );
    context.insert(
        "job_levels",
        &job_level_service::get_all_job_levels(pool).await?,
    );
    context.insert(
        "job_positions",
        &job_position_service::get_all_job_positions(pool).await?,
    );
    context.insert(
        "employment_statuses",
        &employment_status_service::get_all_employment_statuses(pool).await?,
    );
    context.insert("grades", &grade_service::get_all_grades(pool).await?);
    context.insert(
        "ptkp_types",
        &ptkp_type_service::get_all_ptkp_types(pool).await?,
    );
    context.insert(
        "overtime_settings",
        &overtime_setting_service::get_all_overtime_settings(pool).await?,
    );

    context.insert(
        "attendance_locations",
        &attendance_location_service::get_all_locations(pool).await?,
    );
    ...
    context.insert(
        "genders",
        &standard_reference_service::get_items_by_reference_id(pool, "GenderType").await?,
    );
    context.insert(
        "marital_statuses",
        &standard_reference_service::get_items_by_reference_id(pool, "MaritalStatusType").await?,
    );
    context.insert(
        "religion_types",
        &standard_reference_service::get_items_by_reference_id(pool, "ReligionType").await?,
    );
    context.insert(
        "education_levels",
        &standard_reference_service::get_items_by_reference_id(pool, "EducationLevelType").await?,
    );
    context.insert(
        "bank_account_types",
        &standard_reference_service::get_items_by_reference_id(pool, "BankAccountType").await?,
    );
...
pub async fn list_employees(
    ...
    let employees =
        employee_service::get_all_employees_paginated(pool.get_ref(), limit, offset).await?;
...
pub async fn add_employee_action(
    ...
    let new_employee =
        employee_service::create_employee(pool.get_ref(), &form, &created_by).await?;

    Ok(HttpResponse::Found()
        .append_header((
            "Location",
            format!("/employees/edit/{}", new_employee.employee_id),
        ))
        .finish())
}
